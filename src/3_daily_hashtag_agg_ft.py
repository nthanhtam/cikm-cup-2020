...
def train_doc2_vec(
    ...
    window=4,
    workers=8,
    model_path="./models/doc2vec_hashtags_daily.model",
    train_feature_file="",
    valid_feature_file="",
    test_feature_file="",
):
    ...
    features = ["hashtags_d2v_daily_%i" % x for x in range(len(feat[0]))]
    ...
    df = df.merge(df_all, how='left', on='date')
...
def main(
    ...
    train_doc2_vec(
        data_path=os.path.join(data_path, "daily_hashtags.csv.gz"),
        model_path="./models/doc2vec_hashtags_daily.model",
